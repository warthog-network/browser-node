 ...
 *
 * Zip has files at the archive root (manifest.json, app.js, …). Extracting
 * the zip creates one folder — warthog_node_extension/ — that is Load unpacked
 * ready. No inner extra folder.
 *
...
const zipName = 'warthog_node_extension.zip';
const zipPath = path.join(downloadsDir, zipName);
const stagingName = 'warthog_node_extension';
...
function zipStaging() {
  ...
  // Files at zip root so "Extract" (folder named after the zip) is
  // Load-unpacked-ready. An extra inner directory made folder-in-folder.
  const r = spawnSync(
    'zip',
    ['-r', '-q', zipPath, '.', '-x', '*/.DS_Store'],
...
console.log('[package] serve path: /downloads/warthog_node_extension.zip');
console.log('[package] install: unzip → Load unpacked → select warthog_node_extension/');
